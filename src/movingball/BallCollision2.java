...
public class BallCollision2 extends JFrame {
	Painting contentPane = new Painting();
	int width = 700;
	int height = 700;
	
	static final double friction = 0;
	static final double restriction = 0;
			...
			public void run() {
				...
				while(true) {
					...
					if ((X2-X1)*(X2-X1)+(Y2-Y1)*(Y2-Y1) <= (radius1+radius2)*(radius1+radius2)) {
						...
						double dx2temp = dx2;
						double dy2temp = dy2;
						dx1 = ((restriction+1)*dx2-restriction*dx1)/2;
						dy1 = ((restriction+1)*dy2-restriction*dy1)/2;
						dx2 = ((restriction+1)*dx1-restriction*dx2)/2;
						dy2 = ((restriction+1)*dy2-restriction*dy1)/2;
